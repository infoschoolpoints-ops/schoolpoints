"""Fixed thermal printer with proper Hebrew dates and formatting."""
import win32print
import time
...
class HebrewDate:
    ...
    def get_hebrew_date(date=None):
        ...
        # Approximate Hebrew calendar
        # For production, use pyluach library
        day = date.day
        month = date.month
        year = date.year
        ...
        # Hebrew month (approximate - starts from Tishrei)
        hebrew_month_idx = (month + 6) % 12
        hebrew_month = HebrewDate.HEBREW_MONTHS[hebrew_month_idx]
        
        # Convert day and year to Hebrew numerals
        hebrew_day = HebrewDate.number_to_hebrew(day)
        
        # Year in Hebrew (only last 3 digits typically)
...
class ThermalPrinter:
    """Thermal printer for receipts and vouchers."""
    ...
    def print_receipt(self, student_name, student_class, purchases, 
        ...
        # Hebrew date and time only
        now = datetime.now()
        hebrew_date = HebrewDate.get_hebrew_date(now)
        time_str = now.strftime("%H:%M:%S")
        
        date_line = self.reverse_hebrew(hebrew_date)
        ...
        # Purchase items - item and price on same line
        data.extend(b'\x1B\x61\x02')
        total_points = 0
        ...
        for purchase in purchases:
            ...
            # Build line: "נקודות" + points + spaces + name (+ qty if >1)
            ...
            if qty > 1:
                ...
                line = points_text + f" {item_total}" + " " * spacing + self.reverse_hebrew(item_name)
            else:
                spacing = max(1, 20 - len(name))
                line = points_text + f" {item_total}" + " " * spacing + self.reverse_hebrew(name)
    ...
    def print_voucher(self, student_name, student_class, product_name, 
        ...
        """Print a voucher for product/challenge."""
        data = bytearray()
        ...
        # Variation - fix parentheses by NOT reversing them
        if variation:
            data.extend(b'\x1D\x21\x11')
            # Build with correct parentheses: ) + reversed_text + (
            var_line = ")" + self.reverse_hebrew(variation) + "("
        ...
        date_line = self.reverse_hebrew(f"תאריך ביצוע: {hebrew_date}")
        data.extend(date_line.encode('cp862'))
        data.extend(b'\n')
